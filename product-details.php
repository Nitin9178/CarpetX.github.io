...
if(isset($_GET['action']) && $_GET['action']=="add"){
   ...
   {
    $fetch_cart = $con->prepare("SELECT * FROM cart WHERE user_id = ? AND pro_id = ?");
    $fetch_cart->bind_param('ii' , $_SESSION["login"] , $id);
    $fetch_cart->execute();
...
<?php
   ...
   {
       $insert_data = $con->prepare("INSERT IGNORE INTO cart(user_id , pro_id , qty)VALUES(?,?,?)");
       $insert_data->bind_param('iii', $_SESSION['login'] , $id , $qty);
       $insert_data->execute();
       ?>
<script>
alert("Product inserted in the cart");
</script>
<?php
   }
   }
...
{
$fetch_wish = $con->prepare("SELECT * FROM wishlist WHERE userId = ? AND productId = ?");
$fetch_wish->bind_param('ii', $_SESSION['login'] , $pid);
$fetch_wish->execute();
$r = $fetch_wish->get_result();
$res = $r->fetch_assoc();
$user_id = $row['userId'];
...
{
  $insert_wish = $con->prepare("INSERT IGNORE INTO wishlist(userId , productid)VALUES(?,?)");
  $insert_wish->bind_param("ii", $_SESSION['login'] , $pid);
  $insert_wish->execute();
  ?>
<script>
...
{
	$qty=mysqli_real_escape_string($_POST['quality']);
	$price=mysqli_real_escape_string($_POST['price']);
	$value=mysqli_real_escape_string($_POST['value']);
	$name=mysqli_real_escape_string($_POST['name']);
	$summary=mysqli_real_escape_string($_POST['summary']);
	$review=mysqli_real_escape_string($_POST['review']);
    $insert_review = $con->prepare("insert ignore into productreviews(productId,quality,price,value,name,summary,review)values(?,?,?,?,?,?,?)");
    $insert_review->bind_param("iiiisss",$pid , $qty , $price , $value , $name , $summary , $review);
    ...
    <div class="breadcrumb">
        <div class="container">
            <div class="breadcrumb-inner">
                <?php
$ret = $con->prepare("select category.categoryName as catname,subCategory.subcategory as subcatname,products.productName as pname from products join category on category.id=products.category join subcategory on subcategory.id=products.subCategory where products.id= ?");
$ret->bind_param("i" , $pid);
$ret->execute();
$r = $ret->get_result();
    ...
    <div class="body-content outer-top-xs">
        <div class='container'>
            <div class='row single-product outer-bottom-sm '>
                    ...
                    <div class="sidebar-module-container">
                        ...
                        <div class="sidebar-widget hot-deals wow fadeInUp">
                            ...
                            <div class="owl-carousel sidebar-carousel custom-carousel owl-theme outer-top-xs">
                                ...
                                <?php
                                    $ret= $con->prepare("select * from products order by rand() limit 4 ");
                                    $ret->bind_param();
                                    $ret->execute();
                                    $res = $ret->get_result();
                                     while ($rws= $res->fetch_array()) 
                                     {
                                  ?>
                                <div class="item">
                                    <div class="products">
                ...
                <?php 
$ret=$con->prepare("select * from products where id=?");
$ret->bind_param("i",$pid);
$ret->execute();
$r = $ret->get_result();
                                ...
                                <div class="quantity-container info-container">
                                    <div class="row">
                                        ...
                                        <div class="col-lg-5 col-md-5 col-sm-5">
                                            <div class="container">
                                            <?php if($row['productAvailability']=='In Stock'){?>
                                            <a href="product-details.php?action=add&id=<?php echo $row['id']; ?>"
                                                class="btn btn-danger"><i
                                                    class="fa fa-shopping-cart inner-right-vs"></i>ADD TO  CART</a>
                                            <a href="payment-method.php?pro_id=<?= htmlentities($row['id']); ?>">
                                                <button class="btn btn-warning" type="button" style="padding-x:1rem;"><i class="fa fa-bolt inner-right-vs"></i> BUY NOW</button></a>
                                            <?php } else {?>
                                            <div class="action" style="color:red">Out of Stock</div>
                                            <?php } ?>
                                    ...
                                    <ul class="list-unstyled">
                                        <li class="add-cart-button btn-group">
                                            ...
                                            <a href="product-details.php?page=product&action=add&id=<?php echo $rw['id']; ?>"
                                                class="lnk btn btn-primary">Add to cart</a><br>
                                        </li>
